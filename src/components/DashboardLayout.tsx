  ...
  return (
    <div className="flex h-screen bg-slate-50 text-slate-900 overflow-hidden font-sans">
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full relative">
        ...
        {/* Dynamic Header */}
        <header className="h-28 flex items-center justify-between px-12 relative z-20 backdrop-blur-sm">
          <div className="flex items-center gap-8">
            <button 
              onClick={() => setIsSidebarOpen(true)}
              className={cn(
                "p-4 bg-white rounded-2xl shadow-xl border border-slate-50 text-slate-400 transition-all hover:text-slate-900 group",
                isSidebarOpen ? "lg:hidden" : "block"
              )}
            >
              <Menu className="w-6 h-6 transition-transform group-active:scale-95" />
            </button>
            <div className="hidden md:flex items-center bg-white p-3 px-6 rounded-[2rem] shadow-xl border border-slate-50 gap-4 w-[32rem]">
          ...
          <div className="flex items-center gap-6">
            ...
            <button className="relative w-14 h-14 bg-white rounded-[1.5rem] flex items-center justify-center text-slate-400 hover:text-slate-950 shadow-xl border border-slate-50 transition-all hover:-translate-y-1 group">
              <Bell className="w-6 h-6 transition-transform group-hover:rotate-12" />
              <span className="absolute top-4 right-4 w-2.5 h-2.5 bg-brand-blue rounded-full border-4 border-white shadow-sm"></span>
            </button>
            
            <Link 
              to="/dashboard/upload"
              className="px-8 py-4 bg-slate-950 text-white rounded-[1.5rem] font-black text-[11px] uppercase tracking-widest shadow-[0_20px_40px_-15px_rgba(0,0,0,0.3)] flex items-center gap-3 active:scale-95 transition-all group"
            >
              Primary Release <Plus className="w-4 h-4 text-brand-blue group-hover:scale-125 transition-transform" />
            </Link>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide perspective-1000">
          <motion.div 
            ...
            className="p-12 pt-4 max-w-7xl mx-auto w-full"
          >
             <Outlet />
          </motion.div>
        </div>
