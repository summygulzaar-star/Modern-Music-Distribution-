...
export default function MyReleases() {
  ...
  const filteredReleases = releases.filter(r => {
    ...
    const matchesSearch = r.songName?.toLowerCase().includes(search.toLowerCase()) || 
                          r.singerName?.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="space-y-12 pb-24">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
        <div>
           <h1 className="text-5xl font-black font-display tracking-tight uppercase">Discography</h1>
           <p className="text-slate-400 font-medium">Manage and track your global catalog in high definition.</p>
        </div>
        <div className="flex items-center gap-4 bg-white p-2 pl-6 rounded-[2.5rem] shadow-xl border border-slate-50 w-full lg:w-auto">
           ...
           />
           <button className="w-12 h-12 bg-slate-950 text-white rounded-3xl flex items-center justify-center hover:bg-brand-blue transition-colors">
         ...
         <AnimatePresence mode="popLayout">
            {filteredReleases.map((release, i) => (
               <motion.div 
                 ...
                 className="bg-white p-8 rounded-[4rem] shadow-sm border border-slate-100 group hover:shadow-3xl transition-all duration-700 relative flex flex-col"
               >
                  {/* Status Indicator */}
                  <div className="absolute top-10 right-10 z-20">
                     <span className={cn(
                        "px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-[0.2em] shadow-sm backdrop-blur-md",
                  ...
                  {/* Artwork Section */}
                  <div className="relative aspect-square rounded-[3rem] overflow-hidden mb-8 shadow-2xl bg-slate-100">
                     <img 
