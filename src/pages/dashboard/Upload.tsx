         ...
         {STEPS.map((s, i) => (
            <div key={s} className="relative z-10 flex flex-col items-center gap-4">
               <motion.div 
                 ...
                 className="w-10 h-10 rounded-full border-2 flex items-center justify-center font-black font-display text-xs shadow-2xl transition-all"
               >
                 {step > i ? <Check className="w-5 h-5" /> : i + 1}
               </motion.div>
               <span className={cn("text-[9px] font-black uppercase tracking-[0.2em]", step >= i ? "text-brand-blue" : "text-slate-300")}>{s}</span>
            </div>
         ))}
      </div>
