...
export default function App() {
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(auth, async (u) => {
      setUser(u);
      if (u) {
        const adminEmail = "Admin@sk.ji".toLowerCase();
        ...
        if (pDoc.exists()) {
          ...
          if (userEmail === adminEmail && data.role !== "admin") {
            ...
            data.role = "admin";
            // Also ensure in admins collection
            await setDoc(doc(db, "admins", u.uid), { uid: u.uid }, { merge: true });
          }
          setProfile(data);
      ...
      <BrowserRouter>
        <Routes>
          ...
          <Route path="/admin" element={isAdmin ? <AdminLayout /> : <Navigate to="/dashboard" />}>
            ...
            <Route path="plans" element={<AdminPlaceholder title="Subscription Plans" />} />
            <Route path="platforms" element={<AdminPlaceholder title="Platform Controls" />} />
            <Route path="notifications" element={<AdminPlaceholder title="System Notifications" />} />
            <Route path="support" element={<AdminPlaceholder title="Support Tickets" />} />
            <Route path="cms" element={<AdminPlaceholder title="Content Management" />} />
            <Route path="reports" element={<AdminPlaceholder title="Enterprise Reports" />} />
            <Route path="logs" element={<AdminPlaceholder title="Security Logs" />} />
            <Route path="settings" element={<AdminPlaceholder title="Global Settings" />} />
          </Route>
        </Routes>
      </BrowserRouter>
